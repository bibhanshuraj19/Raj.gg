...
function MagneticIcon({ children, href, label }: { children: React.ReactNode; href: string; label: string }) {
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.95 }}
      className="w-10 h-10 rounded-xl bg-surface-container-high flex items-center justify-center text-on-surface-variant hover:text-on-surface hover:bg-surface-container-highest transition-colors duration-300"
    >
      {children}
    </motion.a>
  );
    ...
    <section className="relative max-w-5xl w-full mx-auto px-6 sm:px-8 pt-36 sm:pt-44 pb-20" ref={descRef}>
      <motion.div variants={container} initial="hidden" animate="show">
        ...
        <motion.div variants={item} className="flex items-center gap-3">
          {socials.map((s) => (
            <MagneticIcon key={s.label} href={s.href} label={s.label}>{s.icon}</MagneticIcon>
          ))}
        </motion.div>
      </motion.div>
    </section>
