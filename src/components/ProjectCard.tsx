...
export default function ProjectCard({
  ...
  return (
    <>
      ...
      >
        <div
          className={`p-4 flex flex-col ${gradient} from-primary to-secondary rounded-lg border-1 border-accent shadow-2xl shadow-background items-center`}
        >
          <img
            alt=""
            draggable={false}
          ...
          <p className="text-center text-xl">{cardDescription}</p>
        ...
        <Modal open={modalOpen} setOpen={setModalOpen}>
          ...
          <div className="flex lg:flex-row flex-col justify-between mt-6 px-3">
            {/* Left Column: Title, Role, Timeline */}
              ...
              <div className="flex flex-row gap-2 items-center">
                ...
                {url && (
                  <Link
                    href={url}
                    target="_blank"
                    className="bg-middle hover:bg-secondary duration-300 border-1 border-accent p-1.5 rounded-full"
                  >
                    <svg
            ...
            <div className="flex flex-col">
              ...
              <div className="max-h-[16.5rem] overflow-y-scroll bg-neutral-800 border-1 border-accent rounded-lg p-2">
                {fullDescription.map((desc, i) => (
                  <p
                    key={i}
                    className="sm:text-lg text-base first:mt-0 mt-2 max-w-[28rem]"
                  >
                    {desc}
                  </p>
                ))}
